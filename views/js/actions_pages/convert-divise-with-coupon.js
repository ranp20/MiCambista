	...
	if(frmValidCoupon != "" && frmValidCoupon != null && frmValidCoupon != "null" && frmValidCoupon != undefined){
	    ...
	    processData: false,
			...
			if(e != "" && e != null){
				var r = JSON.parse(e);
				if(r.res == "tp_gen"){
					var coupon = (r.received[0].cupon).toUpperCase();
					coupon_name = r.received[0].cupon;
					var val_buy_at = r.received[0].buy_price;
					var val_sell_at = r.received[0].sell_price;
			  	...
			  	$("#refval_buy_at").html(`<span>S/. </span><span id='v-refRateBuyCurrent'>${val_buy_at}</span>`);
			  	$("#refval_sell_at").html(`<span>S/. </span><span id='v-refRateSellCurrent'>${val_sell_at}</span>`);
				}else if(r.res == "st_disabled"){
					Swal.fire({
		        title: 'Atención!',
		        html: `<span class='font-w-500'>Este cupón ya no está activo actualmente.</span>`,
		        icon: 'warning',
		        confirmButtonText: 'Aceptar'
		      });
				}else if(r.res == "not_adda"){
					Swal.fire({
		        title: 'Atención!',
		        html: `<span class='font-w-500'>No tienes activado este cupón.</span>`,
		        icon: 'warning',
		        confirmButtonText: 'Aceptar'
		      });	
				}else if(r.res == "not_exist"){
					Swal.fire({
		        title: 'Error!',
		        html: `<span class='font-w-500'>El cupón que ingresaste no existe.</span>`,
		        icon: 'warning',
		        confirmButtonText: 'Aceptar'
		      });
				}
				...
				Swal.fire({
	        title: 'Error!',
	        html: `<span class='font-w-500'>Lo sentimos, hubo un error al procesar la información.</span>`,
	        icon: 'error',
	        confirmButtonText: 'Aceptar'
	      });
			}
