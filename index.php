...
<body>
	...
	<script type="text/javascript">
		var prefsColorScheme = window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light';
		var darkQuery = window.matchMedia('(prefers-color-scheme: dark)');

		var switchColorScheme = document.querySelector('#darkmode-toggle');
		const setColorScheme = (theme) => {
			document.documentElement.setAttribute('data-theme', theme);
			localStorage.setItem('theme', theme);
		};

		switchColorScheme.addEventListener('click', () => {
			let switchToColor = localStorage.getItem('theme') === 'dark' ? 'light' : 'dark';
			setColorScheme(switchToColor);
		});

		setColorScheme(localStorage.getItem('theme') || prefsColorScheme);
